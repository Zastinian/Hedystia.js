...
class GuildMemberVerification extends Base {
  constructor(data = {}, guildId, client) {
    ...
    this.fields =
      data.form_fields?.map(
        (o) => new VerificationFormFields(o, this.client)
      ) ?? [];
  }
  ...
  async addFields(fields = []) {
    if (fields.length <= 0)
      throw new RangeError(`Por favor, especifique un campo de formulario`);
    fields.map((o) =>
      this.fields.push(GuildMemberVerification.transformoptions(o))
    );
  ...
  async removeFields() {
    if (this.fields.length <= 0)
      throw new RangeError(`This Form has no fields`);
    this.fields = [];
    return await this.edit({ fields: this.fields });
  }
