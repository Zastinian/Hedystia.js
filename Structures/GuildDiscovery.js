...
class GuildDiscovery extends Base {
  constructor(data = {}, guildId, client) {
    ...
    this.emojiDiscoverabilityEnabled =
      data.emoji_discoverability_enabled ?? null;
    this.published = data.is_published ?? null;
    this.keywords = data.keywords ?? null;
    this.partnerActionedAt = data.partner_actioned_timestamp
      ? new Date(data.partner_actioned_timestamp)
      : null;
    this.partnerActionedTimestamp = this.partnerActionedAt?.getTime() ?? null;
    this.partnerApplicationAt = data.partner_application_timestamp
      ? new Date(data.partner_application_timestamp)
      : null;
    this.partnerApplicationTimestamp =
      this.partnerApplicationAt?.getTime() ?? null;
    this.primaryCategoryId =
      (typeof data.primary_category_id === "number"
        ? GuildPrimaryCategory[data.primary_category_id]
