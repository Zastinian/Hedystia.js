...
class SlashOption {
  constructor(data = {}) {
    ...
    this.channelTypes =
      (data.channel_types ?? data.channelTypes)?.map((o) =>
        SlashOption.transformChannelType(o)
      ) ?? undefined;
  ...
  setChannelTypes(...channelTypes) {
    if (Array.isArray(channelTypes[0])) {
      this.channelTypes = channelTypes[0]?.map((o) =>
        SlashOption.transformChannelType(o)
      );
    } else {
      this.channelTypes = channelTypes?.map((o) =>
        SlashOption.transformChannelType(o)
      );
    }
  ...
  validation() {
    ...
    if (!this.name || this.name?.length < 1)
      throw new RangeError(`Option name must be non empty`);
    if (!this.description || this.description?.length < 1)
      throw new RangeError(`Option description must be non empty`);
    if (typeof this?.name !== "string" || typeof this?.description !== "string")
      throw new TypeError(`Option name or description must be string`);
    if (this.name.length > 32)
      throw new RangeError(
        `Option name must be less than or equal to 32 in length`
      );
    if (this.description.length > 100)
      throw new RangeError(
        `Option description must be less than or equal to 100 in length`
      );
    if (typeof this.required !== "boolean" && this.required)
      throw new TypeError(`Option required must be boolean`);
    if (this.choices?.length > 25)
      throw new RangeError(
        `Option choices must be less than or equal to 25 in length`
      );
    if (typeof this.autocomplete !== "boolean" && this.autocomplete)
      throw new TypeError(`Option autocomplete must be boolean`);
    return;
  }
  ...
  toJSON() {
    ...
    return {
      ...
      channel_types: this.channelTypes?.map((o) =>
        SlashOption.transformChannelType(o)
      ),
