...
class MessageCreate extends BaseAction {
  ...
  async _patch(data) {
    const packet = data.d;
    if (this.client.partials.includes("DM"))
      await this.cacheDm(packet.channel_id, packet.guild_id);
    const channel = this.client.channels._add(packet.channel_id);
    this.cacheMembers(packet, packet.guild_id, channel);
    const message = channel.messages?._add(
      packet,
      packet.guild_id,
      packet.channel_id,
      { cache: true, force: true }
    );
    return this.client.emit("MensajeCreado", message);
  }
  ...
  cacheMembers(members = {}, guildId, channel) {
    ...
    if (members.member) {
      ...
      manager.cache.clear();
      return manager._add(members.member, guildId, {
        force: true,
        cache: true,
      });
    }
