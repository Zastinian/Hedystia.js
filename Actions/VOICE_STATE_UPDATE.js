...
class VoiceStateUpdate extends BaseAction {
  ...
  _patch(data) {
    ...
    const newState = manager._add(packet, packet.guild_id, {
      cache: true,
      force: true,
    });
    this.client.emit("ActualizacionDelEstadoDeVoz", oldState, newState);
    if (packet.channel_id === null)
      return manager.cache.delete(packet.session_id);
  }
}
