...
class ThreadUpdate extends BaseAction {
  ...
  _patch(data) {
    ...
    const newThread = threads._add(packet, packet.guild_id, {
      cache: true,
      force: true,
    });
    this.client.emit("HiloActualizado", oldThread, newThread);
    return this.client.channels._add(packet, packet.guild_id, {
      cache: true,
      force: true,
    });
  }
