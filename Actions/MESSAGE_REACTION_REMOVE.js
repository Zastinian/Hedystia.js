  ...
  async _patch(data) {
    ...
    if (channel.isText()) {
      ...
      if (this.client.partials.includes("MESSAGE"))
        message = await message.fetch({ force: true, cache: true });
      ...
      this.client.emit(
        "MensajeReaccionRemovida",
        this.client.users._add(packet.user_id),
        reaction
      );
      return message.reactions.reactions.delete(
