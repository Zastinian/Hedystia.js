      ...
      if (cachedSticker) {
        ...
        if (!cachedSticker.equals(val))
          this.client.emit(
            "StickerActualizado",
            cachedSticker,
            stickers._add(val, packet.guild_id, { cache: true, force: true })
          );
      } else {
        if (!stickers.cache.has(val.id))
          this.client.emit(
            "StickerCreado",
            stickers._add(val, packet.guild_id)
          );
      }
    }

    for (let sticker of deletedStickers.values()) {
      this.client.emit("StickerEliminado", sticker);
