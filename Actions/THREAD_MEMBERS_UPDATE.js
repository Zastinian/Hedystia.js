...
class ThreadMembersUpdate extends BaseAction {
  ...
  _patch(data) {
    ...
    const removed = new RaidenCol(
      packet.removed_member_ids?.map((o) => [o, threadMemberManager._add(o)])
    );
    if (added.size > 0) {
      return this.client.emit("HiloMiembroAñadido", added);
    }

    if (removed.size > 0) {
      this.client.emit("HiloMiembroRemovido", removed);
