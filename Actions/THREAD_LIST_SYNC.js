...
class ThreadListSync extends BaseAction {
  ...
  _patch(data) {
    ...
    for (let values of packet.channel_ids) {
      ...
      if (cachedChannel) {
        if (cachedChannel.isThread())
          packet.members?.map((o) => [
            o.user_id,
            cachedChannel.members?._add(o, packet.guild_id, values.id),
          ]);
      }
    }
    return this.client.emit(
      "SincronizacionDeListaDeHilos",
      new RaidenCol(
        packet.threads?.map((o) => [o.id, threads._add(o, packet.guild_id)])
      )
    );
  }
}
