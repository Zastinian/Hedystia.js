...
class MessageReactionAdd extends BaseAction {
  ...
  async _patch(data) {
    ...
    if (channel.isText()) {
      ...
      if (this.client.partials?.includes("Message")) message = await message.fetch({cache: true, force: true});
      ...
      if (!message.partial) message.reactions.reactions.set(reaction.emoji?.id ?? reaction.emoji?.name, reaction);
      return this.client.emit("messageReactionAdd", user ?? guild.members._add(packet.member), reaction);
    }
  }
}
